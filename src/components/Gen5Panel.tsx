...
export const Gen5Panel = forwardRef<TimerPanelHandle, Gen5PanelProps>(
  function Gen5Panel({ onPhasesChange, disabled }, ref) {
    ...
    const canCalibrate = useCallback(() => {
      switch (gen5.mode) {
        case Gen5Mode.STANDARD: return secondHit !== null;
        case Gen5Mode.C_GEAR: return delayHit !== null;
        case Gen5Mode.ENTRALINK: return delayHit !== null && secondHit !== null;
        case Gen5Mode.ENTRALINK_PLUS: return delayHit !== null && secondHit !== null && advancesHit !== null;
      }
    ...
    return (
      <div className="timer-panel gen5-panel">
        <FormField label="Mode">
          <EnumSelect values={GEN5_MODES} value={mode} onChange={(v) => update({ mode: v })} disabled={disabled} />
        </FormField>
        <div className="panel-scroll-area">
          <div className="panel-form-group">
            <FormField label="Target Delay" visible={showTargetDelay}>
              <IntInput value={gen5.targetDelay} onChange={(v) => update({ targetDelay: v ?? 0 })} min={0} max={INT_MAX} disabled={disabled} />
            </FormField>
            <FormField label="Target Second">
              <IntInput value={gen5.targetSecond} onChange={(v) => update({ targetSecond: v ?? 0 })} min={0} max={INT_MAX} disabled={disabled} />
            </FormField>
            <FormField label="Target Advances" visible={showTargetAdvances}>
              <IntInput value={gen5.targetAdvances} onChange={(v) => update({ targetAdvances: v ?? 0 })} min={0} max={INT_MAX} disabled={disabled} />
            </FormField>
            <FormField label="Calibration">
              <IntInput value={gen5.calibration} onChange={(v) => update({ calibration: v ?? 0 })} min={INT_MIN} max={INT_MAX} disabled={disabled} />
            </FormField>
            <FormField label="Entralink Calibration" visible={showEntralinkCalibration}>
              <IntInput value={gen5.entralinkCalibration} onChange={(v) => update({ entralinkCalibration: v ?? 0 })} min={INT_MIN} max={INT_MAX} disabled={disabled} />
            </FormField>
            <FormField label="Frame Calibration" visible={showFrameCalibration}>
              <IntInput value={gen5.frameCalibration} onChange={(v) => update({ frameCalibration: v ?? 0 })} min={INT_MIN} max={INT_MAX} disabled={disabled} />
            </FormField>
          </div>
        </div>
        <div className="panel-hit-fields">
          ...
          <FormField label="Second Hit" visible={showSecondHit}>
            <IntInput value={secondHit} onChange={setSecondHit} min={0} max={INT_MAX} allowBlank placeholder="Enter second hit" disabled={disabled} />
          </FormField>
          <FormField label="Advances Hit" visible={showAdvancesHit}>
            <IntInput value={advancesHit} onChange={setAdvancesHit} min={0} max={INT_MAX} allowBlank placeholder="Enter advances hit" disabled={disabled} />
          </FormField>
        </div>
      </div>
    );
