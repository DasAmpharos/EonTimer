...
export const Gen3Panel = forwardRef<TimerPanelHandle, Gen3PanelProps>(function Gen3Panel(
      ...
      <div className="panel-scroll-area">
        <div className="panel-form-group">
          <FormField label="Pre-Timer">
            <IntInput
              value={gen3.preTimer}
              onChange={(v) => update({ preTimer: v ?? 0 })}
              min={0}
              max={INT_MAX}
              disabled={disabled}
            />
          ...
          <FormField label="Calibration">
            <FloatInput
              value={gen3.calibration}
              onChange={(v) => update({ calibration: v })}
              min={INT_MIN}
              max={INT_MAX}
              disabled={disabled}
            />
