...
export const Gen4Panel = forwardRef<TimerPanelHandle, Gen4PanelProps>(function Gen4Panel(
      ...
      <div className="panel-scroll-area">
        <div className="panel-form-group">
          <FormField label="Calibrated Delay">
            <IntInput
              value={gen4.calibratedDelay}
              onChange={(v) => update({ calibratedDelay: v ?? 0 })}
              min={INT_MIN}
              max={INT_MAX}
              disabled={disabled}
            />
          ...
          <FormField label="Calibrated Second">
            <IntInput
              value={gen4.calibratedSecond}
              onChange={(v) => update({ calibratedSecond: v ?? 0 })}
              min={0}
              max={INT_MAX}
              disabled={disabled}
            />
