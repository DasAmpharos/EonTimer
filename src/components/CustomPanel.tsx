...
export const CustomPanel = forwardRef<TimerPanelHandle, CustomPanelProps>(function CustomPanel(
  ...
  const persist = useCallback(
    (updated: PhaseState[]) => {
      ...
      setCustomPhases(
        updated.map(({ unit, target, calibration }) => ({ unit, target, calibration })),
      );
    },
    [setCustomPhases],
  );
  ...
  const createDisplayData = useCallback(() => {
    const models: CustomPhase[] = phases.map(({ unit, target, calibration }) => ({
      unit,
      target,
      calibration,
    }));
