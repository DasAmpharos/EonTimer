...
namespace service::timer {
    const std::shared_ptr<std::vector<int>> SecondTimer::createStages(
        const int targetSecond, const int calibration) const {
    ...
    int SecondTimer::createStage1(const int targetSecond,
                                  const int calibration) const {
        return util::functions::toMinimumLength(targetSecond * 1000 +
                                                calibration + 200);
    }

    int SecondTimer::calibrate(const int targetSecond,
                               const int secondHit) const {
