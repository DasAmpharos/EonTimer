...
class Gen5Timer(Timer[Gen5Model]):
    ...
    def create(self, model: Gen5Model) -> list[float]:
        ...
        match model.mode.get():
            case Gen5Mode.STANDARD:
                return self.second_timer.create(model.target_second.get(), calibration)
            case Gen5Mode.C_GEAR:
                return self.delay_timer.create(model.target_delay.get(), model.target_second.get(), calibration)
            case Gen5Mode.ENTRALINK:
                return self.entralink_timer.create(
                    model.target_delay.get(), model.target_second.get(), calibration, entralink_calibration
                )
    ...
    def calibrate(self, model: Gen5Model):
        match model.mode.get():
            case Gen5Mode.STANDARD:
                if model.second_hit.get() is not None:
                    model.calibration.add(self.get_second_calibration(model))
            case Gen5Mode.C_GEAR:
                if model.delay_hit.get() is not None:
                    model.calibration.add(self.get_delay_calibration(model))
            case Gen5Mode.ENTRALINK | Gen5Mode.ENTRALINK_PLUS:
                if model.second_hit.get() is not None and model.second_hit.get() != model.target_second.get():
                    model.calibration.add(self.get_second_calibration(model))
                if model.delay_hit.get() is not None and model.delay_hit.get() != model.target_delay.get():
                    model.entralink_calibration.add(self.get_entralink_calibration(model))
                if model.mode.get() == Gen5Mode.ENTRALINK_PLUS:
                    if model.advances_hit.get() is not None and model.advances_hit.get() != model.target_advances.get():
